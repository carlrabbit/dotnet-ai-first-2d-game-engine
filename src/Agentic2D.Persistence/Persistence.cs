...
public sealed class PersistentWorldRuntime
{
    ...
    {
        var r = new PersistentWorldRuntime(); r.Entities.Add(PersistentIds.Player, E(PersistentIds.Player, "active", new() { ["component.position"] = "before-door", ["component.presentation-position"] = "0,90" }, [], new() { ["resource.health"] = 10 }));
    ...
    public StateResolution MovePlayerTo(int x, int y, string intent, string correlation) { var tx = "move-transaction." + intent; var player = Entities[PersistentIds.Player]; Entities[PersistentIds.Player] = player with { Components = new SortedDictionary<string, string>(player.Components.ToDictionary(x => x.Key, x => x.Value), StringComparer.Ordinal) { ["component.presentation-position"] = x + "," + y } }; Emit("player.moved", tx, correlation, PersistentIds.Player, PersistentIds.Player); return new(intent, "accepted", null, tx); }
    public StateResolution DamagePlayer(int amount, string sourceEntityId, string intent, string correlation) { var tx = "damage-transaction." + intent; if (amount <= 0) return new(intent, "rejected", "non-positive-damage", tx); var player = Entities[PersistentIds.Player]; if (!player.Resources.TryGetValue("resource.health", out var before) || before <= 0) return new(intent, "rejected", "invalid-health-resource", tx); var after = Math.Max(0, before - amount); var resources = new SortedDictionary<string, int>(player.Resources.ToDictionary(x => x.Key, x => x.Value), StringComparer.Ordinal) { ["resource.health"] = after }; Entities[PersistentIds.Player] = player with { Resources = resources }; Emit("resource.changed", tx, correlation, sourceEntityId, PersistentIds.Player); Emit("entity.damaged", tx, correlation, sourceEntityId, PersistentIds.Player); return new(intent, "accepted", null, tx); }
