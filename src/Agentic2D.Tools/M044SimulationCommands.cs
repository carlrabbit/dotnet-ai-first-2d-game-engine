...
internal static class M044SimulationCommands
{
    ...
    {
        var world = CreateWorld(checkpoint); Prepare(world, checkpoint); var before = world.SequenceForM044(); Continue(world, checkpoint); var final = Capture(world); await File.WriteAllTextAsync(Path.Combine(destination, checkpoint + ".control.json"), JsonSerializer.Serialize(new { processId = Environment.ProcessId, scheduleId = ScheduleId, scheduleFingerprint = ScheduleFingerprint, checkpoint, checkpointMicroseconds = before.clock, preCheckpointEventIds = before.eventIds, finalFingerprint = final.CanonicalSaveFingerprint, finalSequence = world.SequenceForM044().sequence, postCheckpointEventIds = world.Events.Select(x => x.Id).Where(x => !before.eventIds.Contains(x, StringComparer.Ordinal)).ToArray() }, Json)); await output.WriteLineAsync("m044 control: " + checkpoint); return 0;
    }

    private static async Task<int> Producer(string checkpoint, string destination, TextWriter output)
    {
        var world = CreateWorld(checkpoint); Prepare(world, checkpoint); var before = world.SequenceForM044(); var envelope = Capture(world); var path = Path.Combine(destination, checkpoint + ".canonical-save.json"); new CanonicalRuntimePersistenceService().WriteAtomic(path, envelope, ProjectId, ConfigId, ConfigFingerprint, Content(world)); await File.WriteAllTextAsync(Path.Combine(destination, checkpoint + ".producer.json"), JsonSerializer.Serialize(new { processId = Environment.ProcessId, scheduleId = ScheduleId, scheduleFingerprint = ScheduleFingerprint, checkpoint, checkpointMicroseconds = before.clock, savePath = path, saveFingerprint = envelope.CanonicalSaveFingerprint, preCheckpointEventIds = before.eventIds }, Json)); await output.WriteLineAsync("m044 producer: " + checkpoint); return 0;
    }
