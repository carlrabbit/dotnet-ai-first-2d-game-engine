...
internal static class M044SimulationCommands
{
    ...
    {
        var world = CreateWorld(checkpoint); Prepare(world, checkpoint); var before = world.SequenceForM044(); var fixture = Fixture(world, checkpoint); var envelope = Capture(world); var path = Path.Combine(destination, checkpoint + ".canonical-save.json"); new CanonicalRuntimePersistenceService().WriteAtomic(path, envelope, ProjectId, ConfigId, ConfigFingerprint, Content(world)); await File.WriteAllTextAsync(Path.Combine(destination, checkpoint + ".producer.json"), JsonSerializer.Serialize(new { processId = Environment.ProcessId, scheduleId = ScheduleId, scheduleFingerprint = ScheduleFingerprint, checkpoint, checkpointMicroseconds = before.clock, fixture, savePath = path, saveFingerprint = envelope.CanonicalSaveFingerprint, preCheckpointEventIds = before.eventIds }, Json)); await output.WriteLineAsync("m044 producer: " + checkpoint); return 0;
    }
