...
public static class ToolsCli
{
    public static async Task<int> RunAsync(string[] args, TextWriter output, TextWriter error)
    {
        ...
        var m041 = await M041SimulationCommands.RunAsync(args, output, error);
        ...
        var m044 = await M044SimulationCommands.RunAsync(args, output, error);
        if (m044 >= 0) return m044;
        ...
        if (m033 >= 0) return m033;
        var m032 = await M032SimulationCommands.RunAsync(args, output, error);
        if (m032 >= 0) return m032;
