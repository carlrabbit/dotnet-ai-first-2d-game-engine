...
public static class M029AssetWorkbenchCommands
{
    ...
    private static async Task<int> Open(string[] args, bool resume)
    {
        ...
        else
        {
            ...
            session = new Session("agentic2d.asset-workbench-session.v1", sessionId, campaignId, sourceId, profile, candidates, candidates[0], null, 1, "active", DateTimeOffset.UtcNow.ToString("O"), new("agentic2d.asset-preview-ipc.hello.v2", PreviewIpcHost.EndpointForSession(home, sessionId), "connected", 0, "Agentic2D.DebugClient.Raylib", new[] { "render-projection", "animation", "sound-projection", "raylib-adapter", "no-auto-play", "exact-materialization-subject" }), "review-decisions.jsonl", "promotion-plan.json", []) with { CampaignPath = campaignPath };
        }
