...
internal static class M035RaylibSoakSession
{
    ...
    public static int Run(string[] arguments)
    {
        ...
        try
        {
            ...
            {
                schema = "agentic2d.m035.graphical-soak-session.v2", status, targetSeconds = durationSeconds,
                completedSeconds = (long)stopwatch.Elapsed.TotalSeconds, frames,
                sessionId, startedAtUtc = startedAt, finishedAtUtc = DateTimeOffset.UtcNow, seed = "m035-reference-seed", diagnosticsMode = "continuous-bounded",
                capture = Path.GetRelativePath(Path.GetDirectoryName(Path.GetFullPath(output))!, Path.GetFullPath(capture)),
                controlsObserved = controls.ToArray(), input = Path.GetRelativePath(Path.GetDirectoryName(Path.GetFullPath(output))!, Path.GetFullPath(input)),
                adapterReadOnly = true, liveAuthority = "agentic2d.simulation-world.v1", initialFingerprint = initial,
                finalFingerprint = fixture.Fingerprint, simulationInstantMicroseconds = fixture.Instant, transitionCount = fixture.Transitions,
                saveLoadCount = fixture.SaveLoads, requiredControls, operatorWorkflowComplete, samples, earlyTermination = closedEarly,
                workflowOnly, continuationSession = continuationSession is null ? null : Path.GetRelativePath(Path.GetDirectoryName(Path.GetFullPath(output))!, Path.GetFullPath(continuationSession)), continuationFinalFingerprint = continuationFingerprint,
